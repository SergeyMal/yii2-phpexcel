...
class Excel extends \yii\base\Widget {
     ...
     */
    public function executeColumns($models, $columns = [], $headers = [], &$activeSheet = null,): void {
        if ($activeSheet == null) {
        ...
        if ($this->mode == 'export') {
            ...
            if ($this->isMultipleSheet) {
                ...
                foreach ($this->models as $title => $models) {
                    ...
                    $this->executeColumns($models, $this->populateColumns($columns), $headers, activeSheet: $worksheet[$index]);
                    $index++;
                }
            } else {
                $worksheet = $sheet->getActiveSheet();
                $this->executeColumns($this->models, isset($this->columns) ? $this->populateColumns($this->columns) : [], isset($this->headers) ? $this->headers : [], activeSheet: $worksheet);
            }
